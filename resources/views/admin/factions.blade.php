...
<div class="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
    <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-700">
            ...
            <tr>
                <td class="px-4 py-3 font-mono text-blue-400">{{ $faction->slug }}</td>
                <td class="px-4 py-3">{{ $faction->name }}</td>
                <td class="px-4 py-3 text-gray-400">#{{ $faction->torn_faction_id }}</td>
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded text-xs {{ $faction->status === 'active' ? 'bg-green-900 text-green-200' : 'bg-yellow-900 text-yellow-200' }}">
                        {{ $faction->status }}
                    </span>
                </td>
                <td class="px-4 py-3">
