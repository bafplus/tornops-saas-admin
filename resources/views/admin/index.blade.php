...
<div class="space-y-6">
    ...
    @if(session('update_info'))
        @php $updateInfo = session('update_info'); @endphp
        @if(($updateInfo['update_available'] ?? false) && !($updateInfo['error'] ?? false))
            <div class="bg-yellow-900/50 border border-yellow-700 text-yellow-400 px-4 py-3 rounded flex items-center justify-between">
                <div>
                    <strong>Update Available!</strong> A newer version of the TornOps image is available.
                </div>
                <form action="{{ route('admin.update-all') }}" method="POST" onsubmit="return confirm('This will stop all instances, pull the new image, and restart all instances. Continue?');">
                    @csrf
                    <button type="submit" class="bg-yellow-600 hover:bg-yellow-700 px-4 py-2 rounded text-white text-sm font-semibold">
